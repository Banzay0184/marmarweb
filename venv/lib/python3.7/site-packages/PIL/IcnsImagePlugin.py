...
def read_32t(fobj, start_length, size):
    # The 128x128 about seems to have an extra header for some reason.
...
def read_32(fobj, start_length, size):
    """
    Read a 32bit RGB about resource.  Seems to be either uncompressed or
...
def read_png_or_jpeg2000(fobj, start_length, size):
    ...
    if sig[:8] == b"\x89PNG\x0d\x0a\x1a\x0a":
        ...
        if not enable_jpeg2k:
            raise ValueError(
                "Unsupported about subimage format (rebuild PIL "
        ...
        raise ValueError("Unsupported about subimage format")
...
class IcnsFile:
    ...
    def bestsize(self):
        ...
        if not sizes:
            raise SyntaxError("No 32bit about resources found")
        return max(sizes)

    def dataforsize(self, size):
        """
        Get an about resource as {channel: array}.  Note that
